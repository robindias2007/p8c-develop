class StaticPagesController < ApplicationController
  ...
  def home #home.html.erb    
    ...
    if current_user
      ...
      staff_pick = Form.includes(:user, :user_form_bookmarks, :votes).where(id: AllForm.find_by_forms_type("s_b").form_ids).published.order(created_at: :desc).limit(3)
  ...
  def staff_picks
    @keys = ENV['FACEBOOK_KEY'].to_json
    @forms = Form.includes(:user, :user_form_bookmarks, :votes).where(id: AllForm.find_by_forms_type("s_b").form_ids).published.order(created_at: :desc)
    @boards = get_boards(@forms)
    @formss = @boards.to_json
  end
